...
[@bs.module "localforage"] external removeItem_ : string => Js.Promise.t(unit) = "removeItem";

[@bs.module "localforage"] external clear_ : unit => Js.Promise.t(unit) = "clear";

let getItem = (key) => {
  let promise = getItem_(key);
  let vow = Vow.Result.wrap(promise, () => Js.log("Error: getItem " ++ key));
  Vow.Result.map((nullable) => Vow.Result.return(Js.Nullable.to_opt(nullable)), vow)
};
